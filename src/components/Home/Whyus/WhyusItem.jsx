  ...
  return (
    <div className='w-full flex flex-col gap-1 pt-[3em] pb-[2.5em] relative group cursor-pointer' onClick={()=>setToggle(prev=>!prev)}>
        ...
        <div className='flex w-full justify-between items-center ' >
            <h3 className={`text-[1.4em] md:text-[2.7em] font-semibold ${toggle?" text-[--accent-green]":" text-[--light-text]"} group-hover:opacity-50 duration-[400ms] ease-in-out`}>
                {data.question}
            </h3>
            <div className={ ` w-[20px] md:w-[35px]   h-[20px] md:h-[35px] flex shrink-0 relative ${toggle?'rotate-90':'rotate-0'} duration-[400ms] ease-in-out`}>
        ...
        <p className={`text-[1em] md:text-[1.45em] text-[--light-text] opacity-40 ${toggle?"max-h-[100px]":"max-h-0"} duration-[400ms] ease-in overflow-hidden `}>
            {data.answer}
        </p>
    </div>
  )
